 ...
 * that was never closed is reported and raises *expr_error; the macro
 * definition then fails.
 */
static gerbv_instruction_t *
flush_math_ops(gerbv_instruction_t *ip, int *math_op, int *math_op_idx,
	       const char *amacro_name, int *expr_error)
{
    while (*math_op_idx > 0) {
...
parse_aperture_macro(gerb_file_t *fd)
{
    ...
    int expr_error = 0; /* invalid expression; fails the macro definition */
